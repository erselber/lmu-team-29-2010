...
static const qreal bar_thickness = 0.2;
...
void QtLogo::buildGeometry(int divisions, qreal scale)
{
    ...
    RectPrism cross(geom, bt, bt, bt);
    RectPrism stem(geom, bt, bt, bt);

    qreal stem_downshift = (th + bt) / 2.0;
    QVector3D z(0.0, 0.0, 1.0);
    QVector3D x(1.0, 0.0, 0.0);
    cross.rotate(45.0, z);
    cross.translate(QVector3D(0.0, +stem_downshift, 0.0));
    stem.rotate(45.0, z);

    stem.translate(QVector3D(0.0, -stem_downshift, 0.0));

   // RectTorus body(geom, 1.20, 1.30, 1.1, divisions);

    parts << cross.parts << stem.parts ; //cross.parts <<  body.parts
